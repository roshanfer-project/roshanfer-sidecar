...
2. **Ingress-only**: There is **no `PPMQueue`** step for externally originated HTTP requests on the normal ingress HTTP path—pending RPCs live only in **`Ingress`’s deque**, and **`rpc_queue` EGRESS downstream** stays empty so **`protocol_client(false)`** does not stash ingress HTTP into **`PPMQueue`**. Ingress issues **one Credit Request per queued RPC concurrently** (`Ingress::send_credit_request_checker` increments **`credit_requests_on_fly`** and returns that RPC’s id/priority; **`ingress_pre_credit`** loops until the count matches the deque). **`protocol_client(true)` is not used on ingress**; **`ingress_post_credit`** handles grants.
...
1. After TCP READ handling, **`ingress_pre_credit`** loops **`send_credit_request_checker`** and **`send_credit_request`** for every queued RPC that does not yet have an outstanding Credit Request.
2. **`Ingress::enqueue`**: **Admission** is gated by **`ingress_size_cap`** unless **`lb_mode`** (**`drop_rpc` / 503** when the deque would exceed the cap). Adds **`rpc-id`** and **`priority`** headers. Tracks occupancy via **`ingress_mean.update(queue.size())`** on admit and dequeue for AIMD (see **`rpc_flow.md`**, subsection **AIMD cap**).
3. On UDP Credit Grant, **`ingress_post_credit`** (exactly **one** credit): **`Ingress::dequeue`** pops the head RPC (**`credit_requests_on_fly--`**, **`ingress_mean.update(queue.size())`**) → **`send_sub_request`** when present; otherwise **`prepare_credit_return`** / **`0x02`**. Then **`forward` EGRESS UPSTREAM** flushes 503s; then **`ingress_pre_credit`** (no **`PPMQueue`**).
...
2. Pop **`PPMQueue`** and **`send_sub_request`**: fan-out / early-bind by `credit_id`; sequential late-bind (`mesh_late_binding`) pops min priority (`late_binding_type`: FCFS by `req_rcv_time`, or EDF by `deadline`).
...
- **Ingress**: 
  - **Drops**: **Head-drop when the ingress deque reaches `ingress_size_cap`**, unless **`lb_mode`** (**`Ingress::drop_rpc`** → 503). Other roles do not drop.
