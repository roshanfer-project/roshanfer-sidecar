...
All times are **microseconds** (`steady_clock`). The RLP header is **26 bytes** plus the service name; byte `[2]` is `0x00` for a Credit Request and `0x01` for a credit **response**.

## Client → server (Credit Request)

- **Bytes 14–21:** Send time when the client builds the Credit Request (`send_credit_request`). Same clock is used when the credit reply is received.
...
`update_limits` still uses the piggyback on **incoming Credit Requests** (bytes 22–25 on **requests**), then applies its own scaling; that path is independent of the dwell field on **responses**.
