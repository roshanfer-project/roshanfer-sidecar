...
if [ ! -f "${SCRIPT_DIR}/NanoLog/runtime/libNanoLog.a" ]; then
    ...
    # Also need -fPIC.
    CXX=clang++-22 make libNanoLog.a CXX_ARGS="-std=c++17 -stdlib=libc++ -O3 -fPIC" -j$(nproc)
    CXX=clang++-22 make decompressor CXX_ARGS="-std=c++17 -stdlib=libc++ -O3 -fPIC" -j$(nproc)
else
