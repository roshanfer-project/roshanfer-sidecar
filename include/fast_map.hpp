...
class FastMap {
public:
    // Build from a small set of unique keys. Throws on duplicates. Allows empty sets.
    explicit FastMap(const std::vector<std::string>& keys) {
        ...
        for (uint64_t trial = 0; trial < 100000; ++trial) {
            if (try_build(keys, K, /*cap=*/K, /*seed=*/trial)) { seed_ = trial; perfect = true; break; }
        }
        if (!perfect) {
            // Fall back to small power-of-two capacity >= 2*K
            size_t cap = 1;
            while (cap < 2 * K) cap <<= 1;        // 2,4,8,...
            for (uint64_t trial = 0; trial < 100000; ++trial) {
                if (try_build(keys, K, cap, trial)) { seed_ = trial; break; }
            }
        }
    }
    ...
    // Convenience: get value if present, else LOG(FATAL) with all keys (Acquire read).
    T get(std::string_view k) const noexcept {
        if (auto p = find_ptr(k)) return p->load(std::memory_order_acquire);
    ...
    void set(std::string_view k, T v) noexcept {
        ...
        // Key not found - log all present keys and then fatal error
        LOG(INFO) << "Key '" << k << "' not found for set operation. Available keys:";
        for (size_t i = 0; i < slots_.size(); ++i) {
            if (slots_[i].occupied()) {
                LOG(INFO) << "  - '" << slots_[i].key << "' (index: " << i << ")";
            }
    ...
    void add(std::string_view k, T delta) noexcept {
        ...
        // Key not found - log all present keys and then fatal error
        LOG(INFO) << "Key '" << k << "' not found for add operation. Available keys:";
        for (size_t i = 0; i < slots_.size(); ++i) {
            if (slots_[i].occupied()) {
                LOG(INFO) << "  - '" << slots_[i].key << "' (index: " << i << ")";
            }
    ...
    const std::atomic<T>& by_index(size_t idx) const noexcept { return slots_[idx].value; }
    ...
    const std::string& get_key(std::string_view k) const noexcept {
        size_t idx;
        const Slot<T>* s = probe(k, idx);
        if (s && equals(*s, k)) return slots_[idx].key;
        
        // Key not found - log all present keys and then fatal error
        LOG(INFO) << "Key '" << k << "' not found for get_key operation. Available keys:";
        for (size_t i = 0; i < slots_.size(); ++i) {
            if (slots_[i].occupied()) {
                LOG(INFO) << "  - '" << slots_[i].key << "' (index: " << i << ")";
            }
    ...
    template<typename Func>
    void for_each_occupied_index(Func&& func) const {
        for (size_t i = 0; i < slots_.size(); ++i) {
            if (slots_[i].occupied()) {
                func(i);
            }
...
private:
    ...
    uint64_t seed_ = 0;
    size_t size_ = 0;
    size_t mask_ = 0; // cap-1 for power-of-two; ignored for perfect (cap==K but may not be power-of-two)

    // Build with capacity 'cap' and seed. Returns true if successful (no collisions).
    bool try_build(const std::vector<std::string>& keys, size_t K, size_t cap, uint64_t seed) {
        std::vector<Slot<T>> tmp(cap);
        // For perfect mode cap==K we still index with h % cap; for power-of-two cap we can use &mask.
        const bool pow2 = (cap & (cap - 1)) == 0;
        for (const auto& k : keys) {
            const uint64_t h = fnv1a64(k, seed);
            const size_t idx = pow2 ? (h & (cap - 1)) : (h % cap);
            if (tmp[idx].occupied()) return false; // collision -> fail this seed
            tmp[idx].key = k;
    ...
    static inline bool equals(const Slot<T>& s, std::string_view k) noexcept {
        ...
        // If cap==K and build succeeded, this will always match for the correct key.
        // Still, memcmp is required to be airtight.
        // (Remove the fp check if you prefer purely structural verification.)
...
private:
    ...
    // Compute index and return slot pointer (may be empty).
    inline const LocalSlot<T>* probe(std::string_view k, size_t& idx) const noexcept {
        const uint64_t h = fnv1a64(k, seed_);
        if (mask_ != size_t(-1)) { // power-of-two capacity
            idx = h & mask_;
        } else {                    // perfect: cap==size_
    ...
    static inline bool equals(const LocalSlot<T>& s, std::string_view k) noexcept {
        // Fast checks before memcmp
        if (s.len != k.size()) return false;
        // Optional: check a quick fingerprint to avoid memcmp on false hits.
        // Not strictly needed because MPH avoids collisions, but if we fell back
