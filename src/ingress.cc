...
LocalMap<int32_t> make_drop_id(
    const std::unordered_map<std::string, RoutingEntry, TransparentHash,
                             TransparentEqual> &routing) {
...
LocalMap<std::chrono::time_point<std::chrono::steady_clock>>
make_last_admission(
    const std::unordered_map<std::string, RoutingEntry, TransparentHash,
                             TransparentEqual> &routing) {
  std::vector<std::string> services;
...
int64_t Ingress::add_to_be_admitted_or_drop(RPCQueue &rpc_queue,
  ...
  while (new_requests > 0) {
    auto drop_rpc = std::dynamic_pointer_cast<HTTPMessage>(
        std::move(queue.at(service).back()));
    if (!drop_rpc) {
...
void Ingress::update_stats(int32_t new_p50_us, int32_t new_p95_us,
  ...
  VLOG(1) << "Ingress: Updated p50_us to " << new_p50_us << " and p95_us to "
          << new_p95_us << " for service: " << service;
}

size_t Ingress::size(std::string service) { return queue.at(service).size(); }
