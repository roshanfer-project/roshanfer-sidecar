...
RingWrapper::RingWrapper(size_t ring_size) : size(ring_size) {
  // Initialize the ring
  uint32_t flags = IORING_SETUP_SINGLE_ISSUER | IORING_SETUP_SQPOLL;
  int ret = io_uring_queue_init(static_cast<uint32_t>(size), &ring, flags);
  if (ret < 0) {
    throw std::runtime_error("Failed to initialize ring: " +
                             std::string(errno_string(-ret)));
  }
  ...
  if (config.buffer_count > static_cast<size_t>(32 * 1024 * 1024)) {
    LOG(FATAL) << "Buffer count is too large";
  }

  /* allocate mem for sharing buffer ring */
  if (posix_memalign(
          reinterpret_cast<void **>(&ring_buf), static_cast<size_t>(page_size),
          config.buffer_count * sizeof(struct io_uring_buf_ring)) != 0) {
    LOG(FATAL) << "Failed to allocate memory for buffer ring"
               << ", error: " << (errno);
  }

  /* allocate mem for sharing buffer ring */
  if (posix_memalign(reinterpret_cast<void **>(&dn_ring_buf),
  ...
  reg.ring_addr = static_cast<uint64_t>(reinterpret_cast<uintptr_t>(ring_buf));
  reg.ring_entries = static_cast<uint32_t>(config.buffer_count);
  reg.bgid = 0;
  ...
  dn_reg.ring_addr =
      static_cast<uint64_t>(reinterpret_cast<uintptr_t>(dn_ring_buf));
  dn_reg.ring_entries = static_cast<uint32_t>(config.buffer_count);
  dn_reg.bgid = 1;
...
void RingWrapper::add_buffer_to_ring(std::unique_ptr<Buffer> &buffer,
  ...
  io_uring_buf_ring_add(
      ...
      static_cast<uint32_t>(buffer->get_size()),
      static_cast<uint16_t>(buffer->get_index()),
      io_uring_buf_ring_mask(static_cast<uint32_t>(config.buffer_count)), 0);
...
UserData *RingWrapper::get_user_data(struct io_uring_cqe *cqe) {
  auto *ret = static_cast<UserData *>(io_uring_cqe_get_data(cqe));
  if (ret == nullptr) {
...
void RingWrapper::prepare_write(std::shared_ptr<HTTPConnection> conn,
  ...
  io_uring_prep_send(sqe, fd, buffer->data.data(),
                     static_cast<uint32_t>(buffer->get_filled()), 0);
...
void RingWrapper::handle_multishot_recv(std::unique_ptr<Buffer> &buffer,
  ...
  if (std::cmp_less(cqe_res, header_len)) {
    LOG(FATAL) << "Received message smaller than header length";
  }

  size_t payload_len = static_cast<size_t>(cqe_res) - header_len;
