...
void RPCMapper::allocate_rpc(ConnectionType type, uint32_t stream_id, int fd, bool is_http) {
    auto rpc = pool.get_rpc(stream_id, fd, is_http);
...
void RPCMapper::remove_rpc(ConnectionType type, RPCMessage*& rpc) {
    us_map[type][rpc->get_us_fd()].erase(rpc->get_us_stream_id());
    ds_map[type][rpc->get_ds_fd()].erase(rpc->get_ds_stream_id());
    pool.free_rpc(rpc);
    rpc = nullptr;
}