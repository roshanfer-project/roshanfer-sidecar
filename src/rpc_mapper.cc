...
{
    ds_map.emplace(ConnectionType::INGRESS, std::unordered_map<int, std::unordered_map<int32_t,
        RPCMessage*>>());
    ds_map.emplace(ConnectionType::EGRESS, std::unordered_map<int, std::unordered_map<int32_t,
        RPCMessage*>>());
    us_map.emplace(ConnectionType::EGRESS, std::unordered_map<int, std::unordered_map<int32_t,
        RPCMessage*>>());
    us_map.emplace(ConnectionType::INGRESS, std::unordered_map<int, std::unordered_map<int32_t,
        RPCMessage*>>());
}

void RPCMapper::allocate_rpc(ConnectionType type, int32_t stream_id, int fd, HTTP http) {
    auto rpc = pool.get_rpc(stream_id, fd, http);
    if (ds_map.at(type).find(fd) == ds_map.at(type).end()) {
        ds_map.at(type).emplace(fd, std::unordered_map<int32_t, RPCMessage*>());
    }
...
*/
void RPCMapper::route(ConnectionType type, int32_t ds_stream_id, int ds_fd, int32_t us_stream_id, int us_fd) {
    if (us_map.at(type).find(us_fd) == us_map.at(type).end()) {
        us_map[type].emplace(us_fd, std::unordered_map<int32_t, RPCMessage*>());
    }
    us_map.at(type).at(us_fd).emplace(us_stream_id, ds_map.at(type).at(ds_fd).at(ds_stream_id));
    VLOG(1) << "Mapping DS stream id: " << ds_stream_id << " and fd: " << ds_fd
            << " to US stream id: " << us_stream_id << " and fd: " << us_fd;
}

RPCMessage* RPCMapper::get_us_rpc(ConnectionType type, int32_t us_stream_id, int us_fd) {
    try {
...
RPCMessage* RPCMapper::get_ds_rpc(ConnectionType type, int32_t ds_stream_id, int ds_fd) {
    try {
