...
double td_quantile_of(td_histogram_t *h, double val) {
  ...
  if (val == n->mean) {
    // technically this needs to find all of the nodes which contain this value
    // and sum their weight
  ...
  // we want to figure out where along the line from the prev node to this node,
  // the value falls
  node_t *nr = n;
  node_t *nl = n - 1;
  k -= (nl->count / 2);
